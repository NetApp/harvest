...
func (h *Harvest) Start() {
	status, _ := utils.Exec(HarvestHome, HarvestBin, nil, "start")
	fmt.Println(status)
...
func (h *Harvest) StartByHarvestUser() {
	status, _ := utils.Exec(HarvestHome, "sudo", nil, "-u", "harvest", HarvestBin, "start")
	fmt.Println(status)
	time.Sleep(30 * time.Second)
	h.AllRunning()
...
func (h *Harvest) Stop() {
	status, _ := utils.Exec(HarvestHome, HarvestBin, nil, "stop")
	fmt.Println(status)
}
...
func (h *Harvest) GetPollerInfo() []core.Poller {
	...
	harvestStatus, _ := utils.Exec(HarvestHome, HarvestBin, nil, "status")
