...
|------------------|----------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------|
| `client_timeout` | Go duration    | how long to wait for server responses. This must cover streaming a whole `batch_size` page of events, not just the time to first byte, and this endpoint's response time varies by more than 10x for an identical query. Keep it at or below the `data` interval                                                                                                                                                                                          | 1m      |
| `batch_size`     | int            | number of EMS events requested per page, sent as ONTAP's `max_records` query parameter. The cost of this endpoint is per request rather than per record - a request takes roughly the same time whether it returns 300 records or 3000 - so collection time tracks the number of requests, and a larger page is faster. Lowering it is rarely useful: on one 32-node cluster the same poll needed 2 requests and 22s at 10000, against 13-16 requests and 90-169s at 500                                      | 10000   |
...
#### Memory and the collection window

A poll holds every event it collects in memory until the poll finishes. The collector has to order events by time
before processing them, so that a resolving event is handled after the event it clears, and nothing can be exported
until the whole window has been collected. The width of that window is the `data` interval, so both the number of
events in one poll and the memory it needs scale with `data` - on a cluster with a high event rate, shortening `data`
is the lever. `data` is also the time budget for one poll, less the `client_timeout` held back so a request already in
flight can finish, which bounds how many pages a poll fetches. The `bytesRx` and `numCalls` metadata metrics report
what each poll actually downloaded.
