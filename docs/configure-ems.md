...
| `batch_size`     | int            | number of EMS events requested per page (`max_records`). The cost of this endpoint is per request rather than per record - a request takes roughly the same time whether it returns 300 records or 3000 - so collection time tracks the number of requests, and a larger page is faster. Lowering it is rarely useful: on one 32-node cluster the same poll needed 2 requests and 22s at 10000, against 13-16 requests and 90-169s at 500                                      | 10000   |
| `max_lookback`   | Go duration    | furthest back the collector will ask for events. The watermark only advances on a successful poll, so this bounds how wide the window can grow after a run of failures. Events older than this are skipped, and a warning is logged                                                                                                                                                                                                                                            | 15m     |
| `max_records`    | int            | most events collected in a single poll. Bounds memory and parse time on clusters that emit events faster than Harvest can page them. Exceeding it truncates the poll with a warning                                                                                                                                                                                                                                                                                            | 20000   |
