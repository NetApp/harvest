...
| `max_records`    | int            | most events collected in a single poll, counted **across all of the collector's event-name queries rather than per query**. The cap is poll-wide because every record is held in memory at once, sorted, and processed together, so a per-query cap would let one poll hold `queries x max_records`. Reaching it truncates the poll and logs a warning naming the window and the record count; the events already collected are exported as normal                                                                                 | 20000   |
