...
func showApisCurl(ontapSwag ontap) {
	var (
		poller *conf.Poller
		err    error
	)
	...
	if poller, err = conf.GetPoller2(args.Config, args.Poller); err != nil {
		fmt.Printf("poller named [%s] not found\n", args.Poller)
		os.Exit(1)
	}

	apiRest := "https://" + value(poller.Addr, "") + "/api"
	for _, name := range sortApis(ontapSwag.collectionsApis) {
		curlBuilder := strings.Builder{}
		pathItem := ontapSwag.collectionsApis[name]
		curlBuilder.WriteString("curl -k ")
		if args.CurlProxy != "" {
			curlBuilder.WriteString("--proxy ")
			curlBuilder.WriteString(args.CurlProxy)
			curlBuilder.WriteString(" ")
		}
		auth := value(poller.Username, "") + ":" + poller.Password
		curlBuilder.WriteString(`-u "`)
		curlBuilder.WriteString(auth)
		curlBuilder.WriteString(`"`)
		curlBuilder.WriteString(` -H "accept: application/json" "`)
		curlBuilder.WriteString(apiRest)
		curlBuilder.WriteString(name)

		// fetch all
		curlPart := curlBuilder.String()
		fmt.Printf("%s?return_records=true&fields=*\"\n", curlPart)

		sort.SliceStable(pathItem.Get.OperationProps.Parameters, sortParams(pathItem))
		for _, parameter := range pathItem.Get.OperationProps.Parameters {
			if parameter.Name == "" {
				continue
			}
			fmt.Printf("%s?return_records=true&fields=%s\"\n", curlPart, parameter.Name)
		}
	}
}
