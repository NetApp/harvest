...
func (d *Disk) Run(dataMap map[string]*matrix.Matrix) ([]*matrix.Matrix, *collector.Metadata, error) {
					...
					if childObj.IsArray() {
						for _, obj := range childObj.Array() {
							...
							if keys := d.instanceKeys[attribute]; len(keys) != 0 {
								...
								if attributeName == "frus" && attributeValue == "module" {
									shelfChildInstance.SetLabel("module_type", shelf.Get("module_type").ClonedString())
								}
	...
	d.RequestMetadata.PluginInstances.Store(pluginInstances)
		...
		for _, k := range shelfMetrics {
			...
			case "power":
				...
				case v.psuPowerDrawn > 0:
					...
					d.SLogger.Debug(
						"rail classification unavailable; using psu.power_drawn for shelf power",
						slog.String("shelf", instance.GetLabel("shelf")),
