...
func (my *Sensor) calculateEnvironmentMetrics(data *matrix.Matrix) ([]*matrix.Matrix, error) {
	...
	for k, instance := range data.GetInstances() {
		if k == "" {
			my.Logger.Warn().Str("key", k).Msg("missing instance key")
			continue
		}
		...
		for mKey, metric := range data.GetMetrics() {
			if mKey == "value" {
				...
				if criticalLowMetric := data.GetMetric("crit_low"); criticalLowMetric != nil {
					criticalLowThr, _, _ = criticalLowMetric.GetValueFloat64(instance)
				} else {
					criticalLowThr, _, _ = data.GetMetric("critical_low_threshold").GetValueFloat64(instance)
				}
