...
func IsTimestampOlderThanDuration(begin time.Time, timestamp float64, duration time.Duration) bool {
	return begin.Sub(time.UnixMicro(int64(timestamp))) > duration
}
