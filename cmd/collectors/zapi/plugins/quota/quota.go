...
func (my *Quota) Run(data *matrix.Matrix) ([]*matrix.Matrix, error) {
	...
	if my.client.IsClustered() {
		return my.handleCMode(request, data)
	} else {
		return my.handle7Mode(request, data)
	}
}

func (my *Quota) handleCMode(request *node.Node, data *matrix.Matrix) ([]*matrix.Matrix, error) {
	var (
		result *node.Node
		quotas []*node.Node
	...
	for {
		...
		my.Logger.Debug().Msgf("fetching %d quotas", len(quotas))
...
func (my *Quota) handle7Mode(request *node.Node, data *matrix.Matrix) ([]*matrix.Matrix, error) {
	var (
		result *node.Node
		output []*matrix.Matrix
		err    error
	...
	if result, err = my.client.InvokeRequest(request); err != nil {
		return nil, err
	}
